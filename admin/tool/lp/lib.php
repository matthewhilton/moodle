...
function tool_lp_coursemodule_standard_elements($formwrapper, $mform) {
    ...
    MoodleQuickForm::registerElementType('course_competency_overridegrade',
                                         "$CFG->dirroot/$CFG->admin/tool/lp/classes/course_competency_overridegrade_form_element.php",
...
function tool_lp_coursemodule_edit_post_actions($data, $course) {
    ...
    if (isset($data->competency_rule)) {
        ...
        foreach ($current as $coursemodulecompetency) {
            \core_competency\api::set_course_module_competency_ruleoutcome($coursemodulecompetency, $data->competency_rule, $overridegrade);
        }
    }
