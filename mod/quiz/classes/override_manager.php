...
class override_manager {
    ...
    public static function create_from_override($overrideid) {
        $override = quiz_override::get_record(['id' => $overrideid], MUST_EXIST);
        return self::create_from_quiz($override->get('quiz'));
    }
    ...
    public function get_all_overrides() {
        ...
        return quiz_override::get_records(['quiz' => $this->quizid]);
    }
    ...
    public function parse_formdata($formdata) {
        ...
        $settings = array_intersect_key($formdata, array_flip(quiz_override::KEYS));
    ...
    public function upsert_override($formdata): int {
        ...
        $id = $formdata['id'] ?? 0;
        $override = new quiz_override($id, (object) $datatoset);
        $override->validate();
        $override->save();
    ...
    public function delete_override($id, $shouldlog = true) {
        ...
        $override = new quiz_override($id);

        // Delete the events.
        $eventssearchparams = ['modulename' => 'quiz', 'instance' => $this->quizid];

        if (!empty($override->get('userid'))) {
            $eventssearchparams['userid'] = (int) $override->get('userid');
        }

        if (!empty($override->get('groupid'))) {
            $eventssearchparams['groupid'] = (int) $override->get('groupid');
        }
        ...
        // Clear the cache.
        $this->clear_cache($override->get('userid'), $override->get('groupid'));

        // Log deletion.
        if ($shouldlog) {
            $this->log_deleted($id, $override->get('userid'), $override->get('groupid'));
        }
    }
    ...
    private function clear_unused_values($formdata) {
        ...
        foreach (quiz_override::KEYS as $key) {
